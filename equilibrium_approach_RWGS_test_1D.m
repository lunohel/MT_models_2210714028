% Test for validity of equilibrium of RWGS - Luca Nohel 09.2024
% add initial compositions in lines 70 and also 297 -> run..

sf=[1 1 1];

T=1123; % K Reactor Temperature
p=101325; % Pa
mass_cat=0.0756*0.81; % g of Catalyst on coupon

% Inlet volume flows

V=[400]; %ml/min at standard conditions
V_cold=V;
V=V*(T/293)/60*10^(-6); %m3/s at T reaction (calculated form ideal gas)

% Initial guesses & numerical setup---------
...
K_co=2.93*10^(-8)*exp(40600/8.314*(1/273-1/T)); %Pa^(-1)
K_h2o=6.09*10^(-6)*exp(14500/8.314*(1/273-1/T));
K_co2=1.07*10^(-5)*exp(9720/8.314*(1/273-1/T));
K_h2=5.2*10^(-10)*exp(52000/8.314*(1/273-1/T));

k_rwgs=1.78/60*10^6*exp(-97100/(8.314*T)); %mol/(s g)
...
for l=1:length(V)
    ...
    n_co2_ar=n_co2(i,l)-r_dr*dt_dr;
    n_ch4_ar=n_ch4(i,l)-r_dr*dt_dr;
    n_co_ar=n_co(i,l)+2*r_dr*dt_dr;
    n_h2_ar=n_h2(i,l)+2*r_dr*dt_dr;
    n_n2_ar=n_n2(i,l);
    n_h2o_ar=n_h2o(i,l);
    ...
    n_co2_rwgs=n_co2(i+1,l);
    n_h2_rwgs=n_h2(i+1,l);
    n_h2o_rwgs=n_h2o(i+1,l);
    n_co_rwgs=n_co(i+1,l);
    ...
    for h=1:fs
     ...
     gamma_2=(1-(p_co*10^(-5)/1.01325*p_h2o*10^(-5)/1.01325)/(p_h2*10^(-5)/1.01325*p_co2*10^(-5)/1.01325*K_eqrwgs));

     r_rwgs=(k_rwgs*K_co2*p_co2*gamma_2)/...
         (1+K_co2*p_co2+K_h2*p_h2+K_h2o*p_h2o+K_co*p_co);
     
     r_rwgs=r_rwgs*mass_cat/(nz);
     
     n_co2_rwgs=n_co2_rwgs-r_rwgs*dt_rwgs;
     n_h2_rwgs=n_h2_rwgs-r_rwgs*dt_rwgs;
     n_h2o_rwgs=n_h2o_rwgs+r_rwgs*dt_rwgs;
     n_co_rwgs=n_co_rwgs+r_rwgs*dt_rwgs;
     
     co2=n_co2_rwgs/n_sum;
     co=n_co_rwgs/n_sum;
     h2=n_h2_rwgs/n_sum;
     h2o=n_h2o_rwgs/n_sum;
    ...
    h2co_kinetics(l)=h2/co;
    
end

for l=1:length(V)
X_ch4_kinetics(l)=(v(1,l)*n_ch4(1,l)-v(end,l)*n_ch4(end,l))./(v(1,l)*n_ch4(1,l));
end
...
% Scaling factors from kinetic modeling 
sf=[1 1 1];
